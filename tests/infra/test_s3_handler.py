...
class TestS3Handler:
    ...
    def test_given_valid_s3_object_when_downloading_then_succeeds(self, aws_s3_client):
        # Given: S3 object exists in bucket
        test_content = b"test video content"
        aws_s3_client["s3_client"].put_object(
            Bucket=aws_s3_client["test_bucket"],
            Key=aws_s3_client["test_key"],
            Body=test_content,
        )
        ...
        try:
            download_file(
                aws_s3_client["test_bucket"], aws_s3_client["test_key"], temp_path
            )
    ...
    def test_given_nonexistent_key_when_downloading_then_raises_client_error(
        self, aws_s3_client
    ):
        ...
        with pytest.raises(ClientError) as exc_info:
            download_file(
                aws_s3_client["test_bucket"],
                nonexistent_key,
                aws_s3_client["test_file_path"],
            )

        # moto returns "404" instead of "NoSuchKey" for this case
        assert exc_info.value.response["Error"]["Code"] in ["NoSuchKey", "404"]

    def test_given_nonexistent_bucket_when_downloading_then_raises_client_error(
        self, aws_s3_client
    ):
        ...
        with pytest.raises(ClientError) as exc_info:
            download_file(
                nonexistent_bucket,
                aws_s3_client["test_key"],
                aws_s3_client["test_file_path"],
            )

        assert exc_info.value.response["Error"]["Code"] == "NoSuchBucket"

    def test_given_valid_file_when_uploading_then_succeeds(self, aws_s3_client):
        ...
        try:
            ...
            upload_file(
                temp_path, aws_s3_client["test_bucket"], aws_s3_client["test_key"]
            )

            # Then: File should be uploaded successfully
            response = aws_s3_client["s3_client"].get_object(
                Bucket=aws_s3_client["test_bucket"], Key=aws_s3_client["test_key"]
            )
    ...
    def test_given_nonexistent_bucket_when_uploading_then_raises_client_error(
        self, aws_s3_client
    ):
        ...
        try:
            ...
            with pytest.raises((ClientError, S3UploadFailedError)) as exc_info:
                upload_file(temp_path, nonexistent_bucket, aws_s3_client["test_key"])
    ...
    def test_given_paths_with_special_characters_when_downloading_then_handles_correctly(
        ...
        aws_s3_client["s3_client"].put_object(Bucket=bucket, Key=key, Body=test_content)
    ...
    def test_given_paths_with_special_characters_when_uploading_then_handles_correctly(
        ...
        try:
            ...
            upload_file(temp_path, aws_s3_client["test_bucket"], key)

            # Then: Should handle special characters correctly
            response = aws_s3_client["s3_client"].get_object(
                Bucket=aws_s3_client["test_bucket"], Key=key
            )
            uploaded_content = response["Body"].read()
            assert uploaded_content == test_content
        finally:
    ...
    def test_given_unicode_paths_when_downloading_then_handles_correctly(
        self, aws_s3_client
    ):
        ...
        aws_s3_client["s3_client"].put_object(
            Bucket=aws_s3_client["test_bucket"], Key=key, Body=test_content
        )
        ...
        try:
            download_file(aws_s3_client["test_bucket"], key, temp_path)
    ...
    def test_given_successful_download_when_logging_then_logs_info_messages(
        self, mock_logger, aws_s3_client
    ):
        ...
        try:
            ...
            mock_logger.info.assert_any_call(
                f"Downloading s3://{aws_s3_client['test_bucket']}/{aws_s3_client['test_key']} to {temp_path}"
            )
    ...
    def test_given_successful_upload_when_logging_then_logs_info_messages(
        ...
        try:
            ...
            # Then: Should log appropriate messages
            mock_logger.info.assert_any_call(
                f"Uploading {temp_path} to s3://{aws_s3_client['test_bucket']}/{aws_s3_client['test_key']}"
            )
    ...
    def test_given_upload_failure_when_client_error_occurs_then_logs_error_and_raises(
        self, mock_logger, mock_s3_client, aws_s3_client
    ):
        ...
        try:
            ...
            mock_logger.error.assert_called_once_with(
                f"Failed to upload file to s3://{aws_s3_client['test_bucket']}/{aws_s3_client['test_key']}. Error: {mock_error}"
            )
        finally:
            if os.path.exists(temp_path):
                os.unlink(temp_path)
    ...
    def test_given_download_failure_when_client_error_occurs_then_logs_error_and_raises(
        ...
        assert exc_info.value == mock_error
        mock_logger.error.assert_called_once_with(
            f"Failed to download file from s3://{aws_s3_client['test_bucket']}/{aws_s3_client['test_key']}. Error: {mock_error}"
        )
